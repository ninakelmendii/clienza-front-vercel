<template>
  <v-row>
    ...
    <v-row>
      ...
      <v-col cols="12" xs="12" sm="12" md="8" lg="8">
        <v-row justify="end" align="center">
          <v-btn color="primary" @click="uploadFile = true"
    ...
    <v-dialog v-model="uploadFile" max-width="500px" v-if="$helperPlugin.ifAdmin()">
      <v-card>
        <v-card-title>
            ...
            <v-row>
              ...
              >
                <v-file-input
                  ...
                  v-model="files"
                  :placeholder="translate('Select your PDF file')"
        ...
        <v-card-actions>
          <v-spacer></v-spacer>
          <v-btn color="secondary" @click="uploadFile = false"> {{translate('Cancel')}} </v-btn>
    ...
    <v-col cols="12" md="12" lg="12">
      <v-row justify="center" v-if="data.length == 0 && !loading">
        <h3>{{translate("No Data")}}</h3>
      </v-row>
      <v-row>
        <v-col cols="12" md="3" lg="3" v-for="video in data" :key="video.id">
          <v-card outlined class="card-offers">
            ...
            <div class="pa-6">
              <v-img
                class="card-image"
                :src="require('@/assets/images/thumbnails/dokumente.jpg')"
              ></v-img>
...
export default {
  ...
  methods: {
    ...
    visitUrl(url) {
      window.open(`https://clienza-api.rockapps.pw/public/storage/${url}`, "_blank");
    },
    async getItemsTable() {
      this.loading = true;
      
      axios
          .get("/internal_documents?term=" + this.search_string + "&category=document")
          .then(response => {
            console.log(response.data);
            this.data = response.data.data;
            this.totalTableData = response.data.length;
          })
          .catch(err => console.log(err))
      ...
      // }
      // axios
      //   .get("/internal_documents?term=" + this.search_string, {
      //     params: {
      //       sort_by: sortBy[0],
      //       sort: sortType,
      //       page: page,
    ...
    downloadFile(file_info){
      ...
      // axios.get('/customer_document/' + file_name).then(response => {

      //   this.file = window.URL.createObjectURL(new Blob([response.data]));

      //   let documentUrl = document.createElement('dokuemte');
      //   documentUrl = this.file;
      //   documentUrl.setAttribute('download', true);
      //   document.body.appendChild(documentUrl);
      //   documentUrl.click();
