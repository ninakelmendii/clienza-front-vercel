<template>
  <v-row class="mt-15 mb-15">
    <v-col cols="12" sm="12" md="4" v-for="document in documents" v-bind:key="document.ctag">
          <v-card outlined class="card-offers">
            <div class="overlay d-flex justify-center align-center">
              <!-- <v-btn @click="visitUrl(document.webUrl)" class="mr-4" icon><svg width="35" height="35" viewBox="0 0 38 52" fill="none" xmlns="http://www.w3.org/2000/svg">
              ...
              <div v-if="$helperPlugin.ifAdmin()">
                <v-btn @click="deleteItem(document.id)" class="ml-4 mr-4" icon>
              ...
              <v-btn icon @click="downloadFile(document)" class="ml-4" >
            ...
            <div class="pa-6">
              ...
              <v-card-title class="font-weight-bold" style="font-size: 1rem; text-align: center; display: block;">{{
                getSingleFileName(document.filename)
              }}</v-card-title>
            </div>
          </v-card>
    </v-col>
...
export default {
  ...
  methods: {
    ...
    async deleteItem(itemKey){
      try {
        await axios.delete('/internal_documents/' + itemKey);
      } catch (error) {
        console.log(error)
      }
      this.getDownloadUrl();
    ...
    updateStatus(application_id, event){
      axios
          ...
          .then((response) => {
            this.snackbar = {
              message: response.data.message.replace('messages.', ''),
              color: "success",
              show: true,
            };
            console.log(response)
    ...
    async getDownloadUrl() {
      try {
          const request =  await axios.get('/internal_documents?category=documents&customer_id=' + this.$router.currentRoute.params.client_id);

          this.documents = request.data.data;
      } catch (error) {
        console.log(error);
      }
    //  await MGraph.getCustomerDownloadFileLink(this.$router.currentRoute.params.client_id, "documents", false).then(res => {
    //   this.documents = res
    //   console.log(res);
    //  });
    }
