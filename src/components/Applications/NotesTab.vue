          ...
          <v-list-item v-for="(item, i) in items" :key="i">
            ...
            <v-list-item-content>
              <v-row>
                ...
                >
                  <v-list-item-title v-text="item.text" style="font-size: 16px;"></v-list-item-title>
                </v-col>
                <v-col
                  cols="12"
                  xs="12"
...
export default {
  ...
  methods: {
    ...
    getCustomerNotes() {
      axios
        .get("/customernote/" + this.$router.currentRoute.params.client_id)
        .then((response) => {
          console.log("cannotread", response);
          this.items = [];
          for (let $i = 0; response.data.length; $i++) {
            this.items.push({
              text: response.data[$i].note,
    ...
    createCustomerNote() {
      this.create_customer_note.customer_id =
        this.$router.currentRoute.params.client_id;
    ...
    updateCustomerNote() {
      axios
        .patch(
          "/customernote/" + this.$router.currentRoute.params.client_id,
          this.create_customer_note
        )
        .then((response) => {
    ...
    deleteCustomerNote() {
      axios
        .delete("/customernote/" + this.$router.currentRoute.params.client_id)
        .then((response) => {
          this.snackbar = {
            message: response.data.message,
            color: "success",
